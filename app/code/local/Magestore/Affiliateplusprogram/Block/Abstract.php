...
class Magestore_Affiliateplusprogram_Block_Abstract extends Mage_Core_Block_Template {
    ...
    public function getProgramName($row) {
        return sprintf('<a href="%s" title="%s">%s</a>'
                ...
                , $this->__('View Program Description')
                , $row->getProgramName()
        );
    }
    ...
    public function getProgramDetails($row) {
        ...
        } elseif ($row->getCommissionType() == 'cart_fixed') {  // Added By Adam 22/07/2014
            $commissionText = Mage::helper('core')->currency($commission) . ' ' . $this->__('for whole cart');
        } else {
        ...
        } elseif ($row->getSecCommissionType() == 'cart_fixed') {  // Added By Adam 22/07/2014
            $secText = Mage::helper('core')->currency($secCommission) . ' ' . $this->__('for whole cart');
        } else {
...
//            $html .= '<br /><span class="affiliateplus-anchor" title="' . $this->__('View tier level commission amounts') . '" onclick="TINY.box.show(popHtml' . $row->getId() . ',0,0,0,0);return false;">' . $this->__('View Tier Commission') . '</span>';
            $html .= '<br /><span class="affiliateplus-anchor" title="' . $this->__('View tier level commission amounts') . '" onclick="ajaxPopup(null,popHtml' . $row->getId() . ',this);">' . $this->__('View Tier Commission') . '</span>';
        }
