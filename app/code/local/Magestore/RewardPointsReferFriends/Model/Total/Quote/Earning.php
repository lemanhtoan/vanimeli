...
class Magestore_RewardPointsReferFriends_Model_Total_Quote_Earning extends Mage_Sales_Model_Quote_Address_Total_Abstract {
    ...
    public function _prepareDiscountForItema(Mage_Sales_Model_Quote_Address $address, $invited_base_discount, $earningPoints) {
        if ($invited_base_discount < 0.0001)
            return $this;
        ...
        foreach ($items as $item) {
            ...
            if ($item->getHasChildren() && $item->isChildrenCalculated()) {
                foreach ($item->getChildren() as $child) {
                    ...
                    $itemBaseDiscount = $invited_base_discount * $baseItemPrice / $baseItemsPrice;
                    $itemBaseEarn = round($earningPoints * $baseItemPrice / $baseItemsPrice, 0, PHP_ROUND_HALF_DOWN);
        ...
        foreach ($items as $item) {
            ...
            if ($item->getHasChildren() && $item->isChildrenCalculated()) {
                ...
                foreach ($item->getChildren() as $child) {
                    ...
                    $child->setBaseTaxableAmount(max(0, $baseTaxableAmount - $child->getRewardpointsInvitedBaseDiscount()));
                    $child->setTaxableAmount(max(0, $taxableAmount - $child->getRewardpointsInvitedDiscount()));
                ...
                $item->setBaseTaxableAmount(max(0, $baseTaxableAmount - $item->getRewardpointsInvitedBaseDiscount()));
                $item->setTaxableAmount(max(0, $taxableAmount - $item->getRewardpointsInvitedDiscount()));
