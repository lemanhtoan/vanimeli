...
class Magestore_Affiliateplus_Block_Adminhtml_Transaction_Edit_Tab_Form extends Mage_Adminhtml_Block_Widget_Form
{
	protected function _prepareForm(){
		...
		$storeId = (isset($data['store_id']) && $data['store_id']) ? $data['store_id'] : Mage::app()->getStore()->getId();
		...
		if($id && $id > 0) {
			...
			$fieldset->addField('account_email', 'select', array(
				'label'	=> Mage::helper('affiliateplus')->__('Affiliate Account Email'),
				'name'  => 'account_email',
				'class'     => 'required-entry',
				'required'  => true,
				'values' => Mage::helper('affiliateplus')->getAccountList(),
			));
			...
			$totalAmount = isset($data['total_amount']) && $data['total_amount'] ? $data['total_amount'] : 0;
			$fieldset->addField('total_amount', 'text', array(
				'label' => Mage::helper('affiliateplus')->__('Total Amount'),
				'text'	=> '<strong>'.$store->convertPrice($totalAmount, true, true).'</strong>',
				'class'     => 'required-entry',
				'required'  => true,
				'readonly'  => true
			));
