...
class Magestore_Affiliateplus_Block_Adminhtml_Account_Edit_Tab_Form extends Mage_Adminhtml_Block_Widget_Form {
    ...
    protected function _prepareForm() {
        ...
        $fieldset->addField('customize_commission_by', 'select', array(
            'label' => Mage::helper('affiliateplus')->__('Commission by'),
            'name' => 'customize_commission_by',
            'values' => array(
                array('value' => 1, 'label' => Mage::helper('affiliateplus')->__('Money (Affiliate Plus)')),
                array('value' => 2, 'label' => Mage::helper('affiliateplus')->__('Point (Reward Points)'))
            ),
            'onchange' => 'changeCommissionBy(this)',
            'onload' => 'changeCommissionBy(this)',
        ));


        $fieldset->addField('customize_commission_proportion', 'text' , array(
            'label' => Mage::helper('affiliateplus')->__('Commission Proportion'),
            'name' => 'customize_commission_proportion',
            'note' => Mage::helper('affiliateplus')->__('Example: When "Commission Proportion" is 1. If Customer earn affiliate $10, he will receive 10 points'),
        ));
        ...
        if (!isset($data['account_id']) || !$data['account_id']) {
            ...
            $fieldset->addField('customer_account', 'note', array(
                ...
                'text' => '<a href="' . $this->getUrl('adminhtml/customer/edit', array('id' => $data['customer_id'])) . '" 
                              title="' . Mage::helper('affiliateplus')->__('Edit Customer') . '">' .
                                 Mage::helper('core')->escapeHtml($data['name']) .
                          '</a>',
            ));
        }
