...
class Magestore_Affiliateplus_Model_Payment extends Mage_Core_Model_Abstract {
    ...
    protected function _beforeSave() {
        ...
        if (!$this->getData('is_reduced_balance') && $this->getStatus() && $this->getStatus() < 4 && ($this->getStatus() == 3 || Mage::getStoreConfig('affiliateplus/payment/reduce_balance'))
        ) {
    ...
    public function sendMailProcessPaymentToAccount() {
        ...
        if(Mage::getStoreConfig('affiliateplus/payment/reduce_balance') == 1) {
            $balanceFormated = $account->getBalance();
        } else {
            $balanceFormated = $account->getBalance() - $this->getAmount();
        }
        $this->addPaymentInfo()->setAccountName($account->getName())
                ->setAccountEmail($account->getEmail())
                ->setAccountPaypalEmail($account->getPaypalEmail())
                ->setBalanceFormated(Mage::helper('core')->currency($balanceFormated))
                ->setRequestPayment(Mage::helper('core')->currency($this->getAmount()))
                ...
                ->setCreatedTimeFormated(Mage::helper('core')
                        ->formatDate(date('Y-m-d H:i:s', Mage::getSingleton('core/date')->timestamp(time()))), 'medium'
                )
                ->setRequestDateFormated(Mage::helper('core')->formatDate($this->getRequestDate(), 'medium'))
        ;
