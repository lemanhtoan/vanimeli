...
//                $account->setBalance($account->getBalance()+$additionalCommission)->save();
        ...
        try {
            ...
            // Zend_debug::dump($transactionCommission,$commission);
            // Zend_debug::dump($account->getData());die();

            $commission = $commission ? $commission : $this->getCommission();

            // customize Get Point replace Commission by Jacob 09/03/2017

            if (Mage::helper('core')->isModuleEnabled('Magestore_RewardPoints')){

                if ($account->getData('customize_commission_by') == '2'){


                    $rewardPoints = $commission * $account->getData('customize_commission_proportion');

                    $affiliateObject = new Varien_Object();

                    $affiliateObject->setData('rewardpoints_earn',$rewardPoints);

                    $customerReward = Mage::getModel('customer/customer')->load($account->getData('customer_id'));

                    Mage::helper('rewardpoints/action')->addTransaction(
                        'earning_affiliate', $customerReward, $affiliateObject
                    );
